...
def plot_records(record: Record, generated: npt.NDArray | None = None):
    ...
    for row_index, signal in enumerate(record.p_signal.T):
        figure.add_trace(
            go.Scatter(
                y=signal,x=list(range(record.p_signal.shape[0])),
                showlegend=False, line={"color": "blue"}
                ),
            row=row_index + 1, col =1
        )
        if generated is not None:
            figure.add_trace(
                go.Scatter(
                    y=generated,x=list(range(record.p_signal.shape[0])),
